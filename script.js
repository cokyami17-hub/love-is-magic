...
// --- TAMBAHAN UNTUK TANGGAL UPLOAD ---
const elTanggal = document.getElementById("tanggal-upload");
if (item.t && elTanggal) {
    const formatTanggal = new Date(item.t).toLocaleDateString('id-ID', {
        day: 'numeric', month: 'long', year: 'numeric', 
        hour: '2-digit', minute: '2-digit'
    });
    elTanggal.innerText = formatTanggal + " WIB";
} else if (elTanggal) {
    elTanggal.innerText = ""; // Kosongin kalau data lama belum ada tanggalnya
}
// --- BATAS TAMBAHAN ---
...
function tampilkanKomentar(scroll) {
    fetch(databaseURL + fotoAktif + ".json").then(r => r.json()).then(data => {
        ...
        Object.keys(data).forEach(k => {
            ...
            // DITAMBAHIN white-space: pre-wrap; BIAR ENTERNYA KEBACA
        ...
        fetch(databaseURL + "status.json").then(r => r.json()).then(st => {
            ...
            keys.forEach((k, idx) => {
                ...
                h += `<div style="display:flex; justify-content:${isMe ? 'flex-end' : 'flex-start'}; margin-bottom: 8px;">
                        <div style="background:${isMe ? '#0095f6' : '#efefef'}; color:${isMe ? '#fff' : '#000'}; padding:8px 12px; border-radius:15px; max-width:80%; font-size:14px; white-space: pre-wrap;">${i.m}</div>
                      </div>`;
