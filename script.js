...
async function prosesUpload() {
    ...
    try {
        ...
        if (result.success) {
            // FIX: Simpan URL murni apa adanya ke database
...
// 4. GRID & MODAL (FIX PROXY ANTI BLOKIR)
function loadGridOtomatis() {
    fetch(databaseURL + "posts.json").then(r => r.json()).then(data => {
        ...
        Object.keys(data).reverse().forEach(k => {
            ...
            // TRIK AJAIB: PAKE PROXY CDN WSRV BIKIN INDIHOME TEMBUS
            let urlAman = i.url;
            if(urlAman.includes("i.ibb.co")) {
                // Kita bersihin dulu kalau ada sisa error "i.ibb.co.com" dari kodingan salah kemaren
                let linkBersih = urlAman.replace("https://", "").replace("http://", "").replace("i.ibb.co.com", "i.ibb.co");
                
                // Panggil server proxy buat ngambilin fotonya
                urlAman = "https://wsrv.nl/?url=" + linkBersih;
            }
            
            const isVid = i.type === "video" || i.url.includes(".mp4");
            // Parsing 2 parameter URL biar modal tetep jalan normal
            let aksi = i.cap === "PINTU_SURAT" ? "masukKeSurat()" : `bukaModal('${i.url}', '${urlAman}', '${i.cap}', ${isVid})`;
...
// Tambah parameter "urlTampil" biar Firebase tetep kenal ID foto aslinya
function bukaModal(idAsli, urlTampil, cap, isVid) {
    ...
    if(isVid) {
        ...
        cont.innerHTML = `<video src="${urlTampil}" controls autoplay loop style="width:100%"></video>`;
    } else {
        cont.innerHTML = `<img src="${urlTampil}" style="width:100%">`;
    }
