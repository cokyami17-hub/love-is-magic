...
function tambahMomen() {
    ...
    const caption = document.getElementById("inp-cap").value;
    const type = document.getElementById("inp-type").value;

    if(!fileName || !caption) return alert("Isi dulu semua datanya, Ibni!");

    fetch(databaseURL + "posts.json", {
        method: "POST",
        body: JSON.stringify({
            url: fileName, // Nama file yang ada di GitHub
            cap: caption,
            type: type,
            t: Date.now()
        })
        ...
        // Otomatis kirim notif email ke Eka
        notifEmail("Ibni Ganteng 😎", `Sayang, ada momen baru di web kita! ❤️\n\nCek di sini: https://cokyami17-hub.github.io/love`);
        location.reload(); // Refresh biar muncul di grid
    });
}

// 3. Fungsi Load Grid Otomatis (Gantiin Grid Manual)
function loadGridOtomatis() {
    ...
    .then(data => {
        ...
        if(!data) {
            grid.innerHTML = "<p style='padding:20px; color:gray; text-align:center;'>Belum ada momen, upload dulu lewat panel admin!</p>";
            return;
        }
        
        let html = "";
        // Urutin dari momen terbaru (paling atas)
        Object.keys(data).reverse().forEach(key => {
            const item = data[key];
            const isVid = item.type === "video" || item.url.includes(".mp4");
            
            html += `
                <div class="photo-item" onclick="bukaModal('${item.url}', '${item.cap}', ${isVid})">
                    ${isVid ? `<video src="${item.url}"></video>` : `<img src="${item.url}">`}
                </div>`;
        });
        grid.innerHTML = html;
    });
