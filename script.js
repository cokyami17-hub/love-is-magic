...
async function prosesUpload() {
    ...
    try {
        ...
        if (result.success) {
            // TRIK ANTI BLOKIR INDIHOME: Otomatis nambahin .com di belakang i.ibb.co
            let linkAman = result.data.url.replace("i.ibb.co", "i.ibb.co.com");
            simpanKeFirebase(linkAman, caption, "image");
        }
...
function simpanKeFirebase(url, cap, tipe) {
    fetch(databaseURL + "posts.json", {
        ...
        //notifTele(`✅ KONFIRMASI: Email notif sudah dikirim ke Eka!`);
        alert("Momen Berhasil Terkirim! ❤️"); location.reload();
    });
}

// 4. GRID & MODAL (FIX ANTI BLOKIR & UKURAN)
function loadGridOtomatis() {
    fetch(databaseURL + "posts.json").then(r => r.json()).then(data => {
        ...
        Object.keys(data).reverse().forEach(k => {
            ...
            // TRIK ANTI BLOKIR: Ngebenerin link foto lu yang lama biar bisa kebuka juga
            let urlAman = i.url.replace("i.ibb.co", "i.ibb.co.com");
            
            const isVid = i.type === "video" || urlAman.includes(".mp4");
            let aksi = i.cap === "PINTU_SURAT" ? "masukKeSurat()" : `bukaModal('${urlAman}', '${i.cap}', ${isVid})`;
            
            h += `
                <div onclick="${aksi}" style="width: 100%; aspect-ratio: 1/1; position: relative; overflow: hidden; cursor: pointer; border: 1px solid #fff;">
                    ${isVid ? 
                        `<video src="${urlAman}" autoplay loop muted playsinline style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; object-fit: cover;"></video>` : 
                        `<img src="${urlAman}" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; object-fit: cover;">`
                    }
                </div>`;
        });
        grid.innerHTML = h;
...
function bukaModal(src, cap, isVid) {
    // FIX PENTING: Firebase gabisa nerima simbol aneh buat nama database, jadi diubah jadi underscore
...
function kirimKomentar() {
    ...
    fetch(databaseURL + fotoAktif + ".json", { method: "POST", body: JSON.stringify({ user: u, teks: teks, t: Date.now() }) }).then(() => {
        ...
        if(isIbni) kirimEmailKeEka("Ibni", `Bales komen aku dong sayang! ❤️ Link: https://cokyami17-hub.github.io/love-is-magic/`, fotoAktif);
    });
}
